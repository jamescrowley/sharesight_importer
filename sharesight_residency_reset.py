...
def load_and_validate_residency_reset(reset_file_path, transactions_file_path):
...
def _validate_pair(sell, buy):
    ...
    for field in (
        "symbol", "market", "quantity", "price_in_instrument_currency",
        "instrument_currency", "exchange_rate_aud", "exchange_rate_gbp",
...
def _validate_quantities(ordinary_rows, pairs, residency_date):
    ...
    for row in ordinary_rows:
        ...
        if transaction_type not in {
            "BUY", "SELL", "OPENING_BALANCE", "BONUS", "CANCEL", "SPLIT",
            "CONSOLD", "MERGE_CANCEL", "MERGE_BUY",
        }:
            continue
...
def _quantity(row):
    try:
        ...
        raise ValueError(f"Line {row.line_number}: invalid quantity {row.data.get('quantity')!r}") from error
